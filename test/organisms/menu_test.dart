...
void main() {
  group('Menu Widget Tests', () {
    ...
    testWidgets('applies correct MenuItemSize for each MenuSize', (tester) async {
      Future<MenuItem> pumpAndGetMenuItem(MenuSize size) async {
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: Menu(
                items: [MenuItem(icon: Icons.home, label: 'Home', onTap: () {})],
                bottomItems: [],
                size: size,
              ),
            ),
    ...
    testWidgets('selects item when tapped and updates active state',
          ...
          final searchItem = tester.widget<MenuItem>(find.widgetWithText(MenuItem, 'Search'));
          expect(searchItem.isActive, true);

          final homeItem = tester.widget<MenuItem>(find.widgetWithText(MenuItem, 'Home'));
